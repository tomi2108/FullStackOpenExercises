...
const App = () => {
  ...
  const vote = () => {
    let set = [...anecdotes];
    set[selected].votes = set[selected].votes + 1;

    if (anecdotes[bestAnecdote].votes < set[selected].votes) {
      const best = selected;
      setBestAnecdote(best);
    }
    setAnecdotes(set);
  };
  ...
  return (
    <div>
      ...
      {anecdotes[bestAnecdote].anecdote}
      <br />
      has {anecdotes[bestAnecdote].votes} votes
      <br />
    </div>
  );
};
